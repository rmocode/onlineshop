//table selecting data from items and displaying in a table
//press checkout which removes item from items table and adds email to order table with new order number
<html lang="en">
<?php session_start(); ?>
<head>
  <meta charset="UTF-8">
...
$sql = "SELECT product, name, price FROM items where id=:id";
...
$result = $statement->fetch(PDO::FETCH_ASSOC);
  ...
  while($row = $result->fetch_assoc()) {
      echo $row["product"] . $row["name"] . "£" .$row["price"];
  }
?>
...
<table class="tablestyle">
  <form action="transaction.php" method="post">
    ...
    <button type="submit" id="submit">BUY NOW</button>
</form>
</table>
